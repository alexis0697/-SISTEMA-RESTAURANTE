...
<div class="container">
  ...
  <div class="card">
    <div class="card-header bg-info">
      <h3 class="my-0"><?= label("Settings"); ?></h3>
    </div>
    <div class="card-body">
      <div class="row">
        <div class="col-md-12">
          ...
          <ul class="nav nav-tabs">
            ...
            <li class="<?php echo ($tab == 'roles') ? 'active' : ''; ?>"><a href="#roles" data-toggle="tab"><i class="fa fa-user-times" aria-hidden="true"></i> <?= label("Roles"); ?></a></li>
            ...
            <li class="<?php echo ($tab == 'permissions') ? 'active' : ''; ?>"><a href="#permissions" data-toggle="tab"><i class="fa fa-key" aria-hidden="true"></i> <?= label("Permisos por Rol"); ?></a></li>
          </ul>
          ...
          <div class="tab-content">
            ...
            <!-- users tab -->
            <div class="tab-pane fade in <?php echo ($tab == 'roles') ? 'active' : ''; ?>" id="roles">
              <table class="table">
                <tr>
                  <th><b><?= label("Nombre Rol"); ?></b></th>
                  <th><b><?= label("Descripción"); ?></b></th>
                </tr>
                <?php foreach ($roles as $rol) : ?>
                  <tr>
                    ...
                    <td>
                      <div class="btn-group">
                        <?php if ($rol->description !== 'admin') { ?><a class="btn btn-default" href="settings/deleteRol/<?= $rol->id; ?>" data-toggle="tooltip" data-placement="top" title="<?= label('Delete'); ?>"><i class="fa fa-times"></i></a><?php } ?>
                        <a class="btn btn-default" href="settings/editRol/<?= $rol->id; ?>" data-toggle="tooltip" data-placement="top" title="<?= label('Edit'); ?>"><i class="fa fa-pencil"></i></a>
                      </div>
                    </td>
                  </tr>
                <?php endforeach; ?>
              ...
              <button type="button" class="btn btn-add btn-lg" data-toggle="modal" data-target="#AddRol">
                <?= label("Agregar Rol"); ?>
              </button>
            </div>
            <!-- users tab -->
            ...
            <!-- Permissions tab -->
            <div class="tab-pane fade in <?php echo ($tab == 'permissions') ? 'active' : ''; ?>" id="permissions">
              <div class="row">
                <div class="col-md-12" style="text-align: center;">
                  <button id="btnGuardarPermisos" onclick="guardarPermisos();" type="button" class="btn btn-success">Guardar Permisos</button>
                </div>
              </div>
              <div class="row" style="margin-top: 20px">
                <div class="col-md-6">
                  <div class="card" style="padding: 20px 25px;">
                    <?php foreach ($roles as $rol) : ?>
                      <div class="row">
                        <div class="col-md-12">
                          <input onclick="obtenerPermisosPorRol(<?= $rol->id; ?>);" value="<?= $rol->id; ?>" type="radio" id="rbRol<?= $rol->id; ?>" name="radio-group">
                          <label style="cursor: pointer" for="rbRol<?= $rol->id; ?>"><?= $rol->name; ?></label>
                        </div>
                      </div>
                    <?php endforeach; ?>
                  </div>
                ...
                <div class="col-md-6">
                  <div class="card" style="padding: 20px 25px;" id="sectionOptionsMenu">
...
<!-- add rol Modal -->
<div class="modal fade" id="AddRol" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title" id="myModalLabel"><?= label("Agregar Rol"); ?></h4>
      </div>
      <?php echo form_open_multipart('settings/addRol'); ?>
      <div class="modal-body">
        <div class="row">
          <div class="col-md-6">
            <div class="form-group">
              <label for="txtNombreRol"><?= label("Nombre Rol"); ?> *</label>
              <input type="text" name="name" id="txtNombreRol" class="form-control" placeholder="<?= label("Nombre Rol"); ?>" required>
            </div>
          </div>
          <div class="col-md-6">
            <div class="form-group">
              <label for="txtDescripcionRol"><?= label("Descripción"); ?></label>
              <input type="text" name="description" class="form-control" id="txtDescripcionRol" placeholder="<?= label("Descripción"); ?>">
            </div>
          </div>
        </div>
      </div>
...
<script type="text/javascript">
  ...
  function obtenerPermisosPorRol(idRol) {
    $("#loadingimg").show();
    $.ajax({
      url: "<?php echo site_url('permissions/obtenerPermisosPorRol') ?>/",
      type: "POST",
      data: {
        idRol: idRol,
      },
  ...
  function guardarPermisos() {
    var idRol = $("input:radio[name='radio-group']:checked").val();
    var data = [];
    if (idRol != undefined) {
      // $('#sectionOptionsMenu').find('input:checkbox:not(:disabled)').each(function() {
      $('#sectionOptionsMenu').find('input:checkbox').each(function() {
        var objData = {};
        objData["id"] = $(this).attr('id');
        objData["value"] = $(this).is(":checked") ? 1 : 0;
        data.push(objData);
      });
      $("#loadingimg").show();
        $.ajax({
          url: "<?php echo site_url('permissions/guardarPermisos') ?>/",
          type: "POST",
          data: {
          ...
          success: function(data) {
            ...
            if (data == 1) {
              ...
              swal("<?= label("Ocurrio un error al guardar los permisos."); ?>");
            }
          },
          error: function(jqXHR, textStatus, errorThrown) {
            $("#loadingimg").hide();
      ...
      swal("<?= label("Seleccione un rol de usuario."); ?>");
    }
