<div class="container" style="padding-top: 40px">
   <div class="card">
      <div class="card-header bg-info">
         <h3 class="my-0">Reporte Rápido</h3>
      </div>
      <div class="card-body">
         <!-- <div class="row">
            <div class="col-md-5">
               <div class="form-group">
                  ...
                  <div class="input-group margin-bottom-sm">
                     ...
                     <input class="form-control" id="ReporteRapidoRange" type="text" name="ReporteRapidoRange" />
                  </div>
               </div>
            </div>
            <div class="col-md-2">
               <button style="margin-top: 23px;" class="cancelBtn btn btn-picker" type="button" onclick="getReporteRapido()"><?= label('GetReport'); ?></button>
            </div>
         </div> -->
         <div class="row">
            <div class="col-md-4">
               <div class="statCart Statcolor03" style="height: 100px;">
            ...
            <div class="col-md-4">
               <div class="statCart Statcolor02" style="height: 100px;">
                  <i class="fa fa-times" aria-hidden="true"></i>
                  <h2 style="display: inline"><?= $this->setting->currency; ?> <span class="count"><?= $TodayExpences; ?></span></h2><br>
                  <span><?= label('Gastos Hoy'); ?></span>
               </div>
            </div>
            <div class="col-md-4">
               <div class="statCart Statcolor01" style="height: 100px;">
                  <i class="fa fa-users" aria-hidden="true"></i>
                  <h2 style="display: inline"><?= $this->setting->currency; ?> <span class="count"><?= $TodayBalance; ?></span></h2><br>
                  <span><?= label('Balance del Día'); ?></span>
               </div>
            </div>
         </div>
      </div>
         ...
         <div class="row">
            <div class="col-md-6">
               <div class="table-responsive">
                  <table id="tblVentasGastosMes" class="table table-striped table-bordered" cellspacing="0" width="100%">
                     <thead>
                        <tr>
                           <th>Item</th>
                           <th>Mes</th>
                     ...
                     <tbody>
                        <?php $i = 1; ?>
                        <?php foreach ($monthlySalesRows as $fila) : ?>
                           <tr>
                              <td><?php echo $i; ?></td>
                              <td><?= $fila->mes; ?></td>
                              <td><?= $this->setting->currency; ?> <?= number_format((float)$fila->ventas, $this->setting->decimals, '.', ''); ?></td>
                              <td><?= $this->setting->currency; ?> <?= number_format((float)$fila->gastos, $this->setting->decimals, '.', ''); ?></td>
                              <td><?= $this->setting->currency; ?> <?= number_format((float)$fila->ventas - (float)$fila->gastos, $this->setting->decimals, '.', ''); ?></td>
                              <?php $i++; ?>
                           </tr>
                        <?php endforeach; ?>
                     </tbody>
         ...
         <div class="row">
            <div class="col-md-5">
               <div class="form-group">
                  ...
                  <div class="input-group margin-bottom-sm">
                     <span class="input-group-addon RangePicker"><i class="fa fa-calendar fa-fw" aria-hidden="true"></i></span>
                     <input class="form-control" id="ReporteTopPlatosRange" type="text" name="ReporteTopPlatosRange" />
                  </div>
               </div>
            </div>
            <div class="col-md-2">
               <button style="margin-top: 23px;" class="cancelBtn btn btn-picker" type="button" onclick="getReporteTopPlatos()"><?= label('GetReport'); ?></button>
            </div>
         </div>
         <div class="row">
            <div class="col-md-6">
               <div class="table-responsive">
                  <table id="tblVentasProductosTop" class="table table-striped table-bordered" cellspacing="0" width="100%">
                     <thead>
                        <tr>
                           <th>Item</th>
                           <th>Producto</th>
                     ...
                     <tbody>
                        <?php $i = 1; ?>
                        <?php foreach ($monthlySalesByProductsRows as $fila) : ?>
                           <tr>
                              <td><?php echo $i; ?></td>
                              <td><?= $fila->name; ?></td>
                              <td><?= $this->setting->currency; ?> <?= number_format((float)$fila->price, $this->setting->decimals, '.', ''); ?></td>
                              <td><?= $fila->cantidad; ?></td>
                              <td><?= $this->setting->currency; ?> <?= number_format((float)$fila->total, $this->setting->decimals, '.', ''); ?></td>
                              <?php $i++; ?>
                           </tr>
                        <?php endforeach; ?>
                     </tbody>
   ...
   <div class="card">
      <div class="card-header bg-info">
         <h3 class="my-0">Estadísticas de Clientes</h3>
      </div>
      <div class="card-body">
   ...
   <div class="card">
      <div class="card-header bg-info">
         <h3 class="my-0">Reporte de Stocks</h3>
      </div>
      <div class="card-body">
         <div class="row rangeStat">
            <div class="col-md-5">
...
<script>
   ...
   $(function() {
      ...
      $('input[name="ReporteRapidoRange"]').daterangepicker({
         locale: {
            format: 'DD/MM/YYYY'
         }
      });
      $('input[name="ReporteTopPlatosRange"]').daterangepicker({
         locale: {
            format: 'DD/MM/YYYY'
         }
      });
   ...
   function getReporteRapido() {
      console.log("go");
   }

   function getReporteTopPlatos() {
      var Range = $('#ReporteTopPlatosRange').val();
      var start = Range.slice(6, 10) + '-' + Range.slice(0, 2) + '-' + Range.slice(3, 5);
      var end = Range.slice(19, 23) + '-' + Range.slice(13, 15) + '-' + Range.slice(16, 18);
      // ajax delete data to database
      $.ajax({
         url: "<?php echo site_url('reports/getReporteTopPlatos') ?>/",
         type: "POST",
         data: {
            start: start,
            end: end
         ...
         success: function(data) {
            $('#tblVentasProductosTop tbody').html(data);
         },
         error: function(jqXHR, textStatus, errorThrown) {
            alert("error");
         }
   ...
   $(window).scroll(function() {
      $('input[name="ReporteRapidoRange"]').daterangepicker('hide');
   });

   // $('input[name="daterange"]').daterangepicker({locale: {format: 'DD/MM/YYYY'}});
   // $('input[name="daterangeP"]').daterangepicker({locale: {format: 'DD/MM/YYYY'}});
   // $('input[name="daterangeR"]').daterangepicker({locale: {format: 'DD/MM/YYYY'}});
   // $('input[name="ReporteRapidoRange"]').daterangepicker({locale: {format: 'DD/MM/YYYY'}});
   // $('input[name="ReporteTopPlatosRange"]').daterangepicker({locale: {format: 'DD/MM/YYYY'}});
   // var d = new Date().getFullYear();
   // $('#ProductRange').val('01/01/' + d + ' - 12/31/' + d);
   // $('#CustomerRange').val('01/01/' + d + ' - 12/31/' + d);
   // $('#RegisterRange').val('01/01/' + d + ' - 12/31/' + d);
   ...
   <div class="modal-dialog modal-lg" role="document" id="statsModal">
      <div class="modal-content">
         <div class="modal-header">
            ...
            <h4 class="modal-title" id="stats"><?= label("ClientsStats"); ?></h4>
         </div>
