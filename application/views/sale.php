 ...
 <div class="page-header pr-0">
      <div class="">
         <a href="invoices/generateXlsSales">
    ...
    <div class="card">
       <div class="card-header bg-info">
          <h3 class="my-0">Listado de Ventas</h3>
       </div>
       <div class="card-body">
             ...
             <div class="row" style="z-index: 1;margin-bottom: 15px;">
                <div class="col-md-3">
                   <div class="form-group">
                      <label for="txtComprobanteFiltro"><?= label("Número Comprobante"); ?>: </label>
                      <input type="text" class="form-control" id="txtComprobanteFiltro" name="txtComprobanteFiltro">
                   </div>
                </div>
                <div class="col-md-3">
                   <div class="form-group">
                      <label for="txtClienteFiltro">Nombre <?= label("Customer"); ?>: </label>
                      <input type="text" class="form-control" id="txtClienteFiltro" name="txtClienteFiltro">
                   </div>
                </div>
                <div class="col-md-3">
                   <div class="form-group">
                      <label for="cbmTipoCompFiltro"><?= label("ComprobanteType"); ?></label>
                      <select class="form-control" id="cbmTipoCompFiltro" name="cbmTipoCompFiltro">
                         <option value=''><?= label("All"); ?></option>
                         <option value="999">Ticket de Consumo</option>
                      </select>
                   </div>
                </div>
                <div class="col-md-3">
                   <div class="form-group">
                      <label for="cmbEstadoFiltro"><?= label("Status"); ?> de Venta: </label>
                      <select class="form-control" id="cmbEstadoFiltro" name="cmbEstadoFiltro">
                         <option value=''><?= label("All"); ?></option>
                         <option value="1" <?= $typeF === '0' ? 'selected' : ''; ?>><?= label("paid"); ?></option>
                         <option value="2" <?= $typeF === '1' ? 'selected' : ''; ?>><?= label("unpaid"); ?></option>
                         <option value="3" <?= $typeF === '2' ? 'selected' : ''; ?>><?= label("Partiallypaid"); ?></option>
                         <option value="4" <?= $typeF === '3' ? 'selected' : ''; ?>><?= label("Anulado"); ?></option>
                      </select>
                   </div>
                </div>
             </div>
          ...
          <div class="row">
             <div class="col-md-12">
                <div class="table-responsive">
                   <table id="table" class="table table-striped table-bordered table_sales" cellspacing="0" width="100%">
                      <thead class="thead-inverse">
                         <tr>
                            <th><?= label('ComprobanteType'); ?></th>
                            <th><?= label('Number'); ?></th>
                            ...
                            <th><?= label('DateCreatedSale'); ?></th>
                            <th><?= label('TotalItems'); ?></th>
                            <th><?= label('Status'); ?></th>
                            <th style="width: 75px;text-align: center;"><?= label('Action'); ?></th>
                         </tr>
                      </thead>
                      <tbody>
                      </tbody>
 ...
 <script type="text/javascript">
    ...
    $(document).ready(function() {
       ...
       table = $('#table').DataTable({
          ...
          "ajax": {
             ...
             "type": "POST",
             "data": function(data) {
                data.txtComprobanteFiltro = $('#txtComprobanteFiltro').val();
                data.txtClienteFiltro = $('#txtClienteFiltro').val();
                data.cbmTipoCompFiltro = $('#cbmTipoCompFiltro').val();
                data.txtEstadoFiltro = $('#cmbEstadoFiltro').val();
    ...
    function canceled_invoice(id) {
       swal({
             ...
             showCancelButton: true,
             confirmButtonColor: "#DD6B55",
             confirmButtonText: '<?= label("yesiamCanceled"); ?>',
             closeOnConfirm: false
          },
          function() {
             // ajax delete data to database
             $.ajax({
                url: "<?php echo site_url('invoices/ajax_canceled') ?>/" + id,
                type: "POST",
                dataType: "JSON",
                success: function(data) {
                   //if success reload ajax table
             ...
             swal('<?= label("Anulado"); ?>', '<?= label("Canceledmessage"); ?>', "success");
          });
    }
