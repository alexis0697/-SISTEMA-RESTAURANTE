   ...
   <div class="modal fade" id="login-modal" tabindex="-1" role="dialog">
      <div class="modal-dialog">
         <div class="loginmodal-container">
            <?php if ($this->setting->logo) { ?><img src="<?= base_url() ?>files/Setting/<?= $this->setting->logo; ?>" alt="logo" style='max-height: 100px; max-width: 100px; margin: 0 auto'><?php } else { ?><img src="<?= base_url() ?>assets/img/logo.png" alt="logo"><?php } ?>
            <h1><?= label('Loginaccount'); ?></h1><br>
            <?php if (isset($message)) {
               echo "<div class='red' style='margin-bottom: 15px'>" . $message . "</div>";
            } ?>
            <?php
            $attributes = array('class' => 'login');
            echo form_open('login', $attributes);
            ?>
            <input type="text" autofocus name="username" value="<?= isset($username) ? $username : 'admin' ?>" placeholder="<?= label("Username"); ?>" required>
            <input type="password" name="password" placeholder="<?= label("Password"); ?>" value="password" required>
            <?php
            ...
            <div class="login-help">
               Copyrights &copy; <?= date("Y"); ?>. All rights reserved by <?= $this->setting->companyname ?>
            </div>
         </div>
      </div>
   </div>
