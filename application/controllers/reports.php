...
class Reports extends CI_Controller
{
    ...
    public function getCustomerReport()
    {
        ...
        $totals = 0;
        $sales = Sale::find_by_sql("SELECT * FROM `zarest_sales` WHERE canceled = 0 AND client_id = '$client_id' AND created_at between '$start' AND '$end' ORDER BY created_at");
        ...
        foreach ($sales as $sale) {
            $result .= '<tr><td>' .  sprintf("%08d", $sale->id) . '</td><td>' . $sale->clientname . '</td><td>' . $sale->discount . '</td><td>' . $this->setting->currency . ' ' . number_format((float)$sale->total, $this->setting->decimals, '.', '') . '</td><td>' . $sale->created_by . '</td><td>' . $sale->totalitems . '</td></tr>';
            $totals += $sale->total;
        }
        $result .= '</tbody>
      </table>
    ...
    public function getDailyPartialCutsOffReport()
    {
        $item = 1;
        $result="";
        $start = $this->input->post('start');
        ...
        $flag = false;
        try {
            $dailySales = Sale::find_by_sql("SELECT closed_at as fecha_corte, cash_total as amount, cash_sub as monto_cortado, delivery, note, closed_by FROM `zarest_registers` WHERE closed_at between '$start' AND '$end' ");
            if(count($dailySales)!=0){
            $result = '<table id="tblVentasDia" class="table table-striped table-bordered" cellspacing="0" width="100%">
        <thead>
           <tr>
    ...
    public function getReporteTopPlatos()
    {
        $start = $this->input->post('start');
        $end = $this->input->post('end');
