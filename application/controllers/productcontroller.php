...
class Productcontroller extends CI_Controller
{
    ...
    public function add()
    {
      ...
      if($this->input->post('type') === '0'){
         $content .= '<div class="col-md-12"><table class="table table-striped"><thead><tr><th width="40%">'.label("Store").'</th><th width="30%">'.label("Stock Inicial").'</th><th width="30%">'.label("price").'</th></tr></thead><tbody class="itemslist">';
         foreach ($stores as $store) {
            $content .= '<tr><td>'.$store->name.'</td><td><input type="number" id="quantity" store-id="'.$store->id.'" value="0"></td><td><input type="number" id="pricestr" store-id="'.$store->id.'" value="'.$this->input->post('price').'" disabled></td></tr>';
         }
         ...
         $content .= '<script type="text/javascript">const e = new Event("change");const element = document.querySelector("#pricestr");element.dispatchEvent(e);</script>';
        ...
        if ($this->upload->do_upload()) {
            ...
            if (!$this->input->post('code')) {
               Product::update_all(array(
                  'set' => array(
                      'code' => sprintf("%08d", $product->id)
                  ),
                  'conditions' => array(
                      'id = ?',
                      $product->id
            ...
            $data = array(
               "type" => $this->input->post('type'),
                ...
                "photothumb" => "default-product.png",
                "created_at" => $date,
                "modified_at" => $date
            );
            $product = Product::create($data);
    ...
    public function Viewproduct($id)
    {
      ...
      $content = '<div class="col-md-6"><div class="media"><div class="media-left">'.$photo.'</div><div class="media-body"><h1 class="media-heading">'.$product->name.'</h1><b>'.label("Category").' :</b> '.$product->category.' <br><b>'.label("Cost").' :</b> ' . $this->setting->currency . ' ' .$product->cost.'<br><b>'.label("ProductTax").' :</b> '.$product->tax.' <br><b>'.label("Price").' :</b> ' . $this->setting->currency . ' ' .$product->price. ' <br><b>'.label("Suppliers").' :</b> '.$product->supplier.' <br><b>'.label("ProductDescription").' :</b> '.$product->description.'</div></div></div><div class="col-md-6"><table class="table"><thead><tr><th width="40%">'.label("Store").'</th><th width="20%">'.label("Quantity").'</th><th width="20%">'.label("price").'</th><th width="20%"><i class="fa fa-eye" aria-hidden="true"></i></th></tr></thead><tbody class="itemslist">';
       foreach ($stores as $store) {
          if($product->type == '0'){
             $stock = Stock::find('first', array('conditions' => array('store_id = ? AND product_id = ?', $store->id, $id)));
             $quantity = $stock ? $stock->quantity : '0';
   ...
   public function barcode($row, $num, $productBcode)
   {
      ...
      $width = 2;
      for ($i=0; $i < $num; $i++) {
         $content .= '<div class="col-md-'.$row.'"><img style="margin-top:30px" src="' . site_url('productcontroller/GenerateBarcode/' . sprintf("%08d", $productBcode) . '/' . $bcs . '/' . $height . '/' . $width) . '" alt="'.$productBcode.'" /></div>';
      }
